...
load(here("kmeans/GO/CD8_unstim_labfilter_c5allv7_enriched_data.rda"))
outdir="kmeans/GO"
...
ggsave(filename=here(outdir, "CD8_unstim_labfilter_cmeansfisherplots2.pdf"), height=15, width=20)
...
df_mtx <- t(df_currated_radar[,-1])
colnames(df_mtx) <- df_currated_radar$geneset
radar_mtx <- rbind(rep(4,length(df_currated_radar$geneset)), rep(1, length(df_currated_radar$geneset)), df_mtx) %>%
  as.data.frame()

radarchart(radar_mtx, 
           axistype=1)

ggplot(df_currated, aes(x=k.5.cluster, y=geneset)) +
...
ggsave(filename=here(outdir, "CD8_unstim_labfilter_cmeanscolorfisherplot.pdf"), height=7, width=10)
...
ggsave(filename=here(outdir, "CD8_unstim_labfilter_widefisherplot.pdf"), height=7, width=15)

plots[[1]]
ggsave(filename=here(outdir, "CD8_unstim_labfilter_cluster1fisher.pdf"), height=4, width=4)
plots[[2]]
ggsave(filename=here(outdir, "CD8_unstim_labfilter_cluster2fisher.pdf"), height=4, width=4)
plots[[3]]
ggsave(filename=here(outdir, "CD8_unstim_labfilter_cluster3fisher.pdf"), height=4, width=4)
plots[[4]]
ggsave(filename=here(outdir, "CD8_unstim_labfilter_cluster4fisher.pdf"), height=4, width=4)
plots[[5]]
ggsave(filename=here(outdir, "CD8_unstim_labfilter_cluster5fisher.pdf"), height=4, width=4)




################################################################################################
#Investigating cluster 4 genes

clus4 <- arrange(df_clus_polish[[4]], desc(neglog10P))
clus4_top10genes <- clus4$genes[1:10]